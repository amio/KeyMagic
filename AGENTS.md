...
- **Shortcuts**: Each `Shortcut` has a `KeyCombo`, a `ShortcutAction` (launch app / run inline script / run script file), and metadata (`isEnabled`, `createdAt`, `modifiedAt`, `lastTriggeredAt`). Application shortcuts are launchers, not visibility toggles: running targets are unhidden and focused, while missing targets are started through Launch Services.
...
- **Action execution**: `Sources/TapTickKit/Services/ShortcutExecutor.swift` — app launch/focus via activation-capable running instances or Launch Services, plus inline script and script-file execution.
- **Settings UI**: `Sources/TapTickKit/Views/SettingsView.swift` (sidebar nav) → `GeneralSettingsView`, `ApplicationsView`, `ScriptsView`, `UtilitiesView`; `UtilitiesView` owns the scalable utility directory, the keystroke overlay settings panel, and the Capture & Mark settings panel.
