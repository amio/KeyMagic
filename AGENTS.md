...
- **Script Execution Logging**: Hotkey-triggered script executions capture stdout+stderr via `Pipe`, store the last `ScriptExecutionLog` per shortcut ID in an in-memory `ScriptLogStore`, and display non-empty output as a subtitle-style floating overlay (`ScriptOutputPresenter`) near the bottom-center of the screen; the Scripts settings tab exposes an "Output" button to review the last captured log for debugging.
...
- **Action execution**: `Sources/TapTickKit/Services/ShortcutExecutor.swift` — app hide/focus/launch via activation-capable running instances with Launch Services fallback, plus inline script and script-file execution; captures script output and fires `onScriptCompleted` callback with a `ScriptExecutionLog`.
- **Script logging & subtitle**: `Sources/TapTickKit/Services/ScriptLogStore.swift` — `ScriptExecutionLog` model and `@Observable ScriptLogStore` keeping the last log per shortcut (in-memory only). `Sources/TapTickKit/Services/ScriptOutputPresenter.swift` — borderless `NSPanel` subtitle overlay (dark translucent pill, monospaced text, 4 s hold + fade-out) positioned at ~15 % from screen bottom, red-tinted background for errors.
