...
PROJECT   := TapTik.xcodeproj
SCHEME    := TapTik
BUILD_DIR := build

# Keychain profile name created via:
#   xcrun notarytool store-credentials "TapTik" --apple-id ... --team-id ... --password ...
# Override on the command line: make dist NOTARIZE_PROFILE=MyOtherProfile
NOTARIZE_PROFILE ?= TapTik
...
# generate so Xcode can resolve TapTikKit.
_patch-pbxproj:
	@perl -i -p0e \
	    's/(isa = XCSwiftPackageProductDependency;\n)(\t+productName = TapTikKit;)/$$1\t\t\tpackage = $(PKG_REF_UUID) \/* XCLocalSwiftPackageReference "." *\/;\n$$2/' \
...
run: ## Build (Debug) and run app from the CLI
	...
	@echo "→ Launching app..."
	@APP_PATH="$(BUILD_DIR)/Build/Products/Debug/TapTik.app"; \
	if [ -d "$$APP_PATH" ]; then \
	    open "$$APP_PATH"; \
	else \
	    BIN="$$APP_PATH/Contents/MacOS/TapTik"; \
	    if [ -x "$$BIN" ]; then \
	        exec "$$BIN"; \
	    else \
...
#   4. Run `make gen` to update TapTik.xcodeproj
...
dist: export notarize dmg ## Full distribution pipeline: archive → export → notarize → staple → DMG
	@echo "  ✓ Distribution build ready: $(BUILD_DIR)/TapTik.dmg"
...
dmg: ## Package the notarized .app into a distributable DMG
	...
	create-dmg \
	    ...
	    --window-size    660 400 \
	    --icon-size      128 \
	    --icon           "TapTik.app" 180 170 \
	    --hide-extension "TapTik.app" \
	    --app-drop-link  480 170 \
	    "$(BUILD_DIR)/TapTik.dmg" \
	    "$(EXPORT_PATH)/$(SCHEME).app"
	@echo "  ✓ DMG: $(BUILD_DIR)/TapTik.dmg"
