...
- Inject only the package-internal async script-running function. This keeps production on `ShortcutExecutor.executeScript` while making execution order and overlap deterministic in controller tests without inventing a scheduler or clock abstraction.
