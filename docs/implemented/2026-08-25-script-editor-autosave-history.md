...
`ScriptEditView` owns draft and autosave state. `ScriptTextEditor` owns the AppKit text boundary, and `ScriptTextUndoController` owns the per-editor `UndoManager` plus observable button state. `ShortcutStore.updateScript` persists only name and action while preserving the latest unrelated shortcut fields.

### Editor Boundary

`ScriptTextEditor` wraps a plain-text `NSTextView`. The text view enables native undo and receives its dedicated manager through `NSTextViewDelegate.undoManager(for:)`. Buttons call that manager directly, while standard keyboard commands reach the same manager through AppKit. External binding updates temporarily disable undo registration so initial loads and model refreshes do not pollute the stack.
